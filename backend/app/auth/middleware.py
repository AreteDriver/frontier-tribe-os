...
async def require_role(required_roles: list[str], member: Member = Depends(get_current_member)) -> Member:
...
def require_leader_or_officer(member: Member = Depends(get_current_member)) -> Member:
    if member.role not in ("leader", "officer"):
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Leader or officer role required")
    return member
